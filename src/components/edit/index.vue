...
<template>
  <div class="px-52 py-12">
    <p class="text-xl font-semibold underline">Add Sales Order Information</p>
    <div class="flex flex-col gap-3 mt-3">
      <input v-model="entries.soNumber" @input="_handleSoNumberInput"
        :class="{ 'focus:border-blue-300 focus:border-2': !isSoNumberInvalid }"
        class="w-1/2 p-3 border-2 border-black placeholder:p-2 rounded-lg" type="text" placeholder="SO NUMBER">
      <input v-model="entries.namaCustomer" class="w-1/2 p-3 border-2 border-black placeholder:p-2 rounded-lg" type="text"
                            ...
                            <tbody>
                              <tr v-for="(rowColor, rowIndexColor) in colorNameData" :key="rowIndexColor"
                                ...
                                <td class="border border-black py-2 text-center"
                                  ...
                                  <input type="number" class="border border-black p-2"
...
const isSubmitDisabled = computed(() => {
  const hasEmptyColorName = entries.value.some(entry => entry.row.some(row => row.colorName.length === 0));
  console.log(hasEmptyColorName, 'empty color')
  const hasNonEmptyRows = entries.value.every(entry => entry.row.length > 0);
  console.log(hasNonEmptyRows, 'non rows');
  return !hasNonEmptyRows || hasEmptyColorName;
});
console.log(entries.value)


const sendData = async () => {
  dataToPost.value.soNumber = entries.value.soNumber
  dataToPost.value.namaCustomer = entries.value.namaCustomer
  dataToPost.value.date = entries.value.date
  dataToPost.value.data = entries.value
  for (let i = 0; i < computedData.value.length; i++) {
    dataToPost.value.data[i].qty = computedData.value[i].qty
  }

  if (!isSubmitDisabled.value) {
    console.log(dataToPost.value);
    const response = await fetch(`http://localhost:8000/api/sales_orders/new/${route.params.id}`, {
      headers: {
        'Content-Type': 'application/json'
      },
      method: 'PUT',
      body: JSON.stringify(dataToPost.value)
    })
    if (response.ok) {
...
const closeModalColorName = async (index, rowIndex) => {
  entries.value[index].row[rowIndex].colorName = []
...
const closeModalMethod = async (index) => {
  isModalMethodOpen.value = false;
  entries.value[index].row = [];
...
const fetchStyleData = async () => {
  try {
    ...
    if (response.ok) {
      const responseData = await response.json();
      styleData.value = responseData.data;
    } else {
      console.error('Failed to fetch style data:', response.statusText);
    }
  } catch (error) {
...
const fetchMethodData = async () => {
  try {
    const response = await fetch('http://localhost:8000/api/color_method');
    if (response.ok) {
      const responseData = await response.json();

      methodData.value = responseData.data
    } else {
      console.error('Failed to fetch style data:', response.statusText);
    }
  } catch (error) {
...
const saveModalStyle = async () => {
  const selectedStyles = styleData.value.filter((row) => row.isSelected);

  const entryNames = entries.value.map((entry) => entry.name_style);

  entries.value.forEach((existingEntry, index) => {
    const isStyleSelected = selectedStyles.some(
      (selectedStyle) => selectedStyle.style_name === existingEntry.name_style
    );
  ...
  selectedStyles.forEach((selectedStyle) => {
    const styleName = selectedStyle.style_name;

    if (!entryNames.includes(styleName)) {
      entries.value.push({
        id: selectedStyle.id,
